...
def mailgun_send_message(domain: str, *, recipient: str, subject: str) -> dict[str, Any]:
    msg_id = (
        "<"
        + hashlib.sha256(f"{domain}|{recipient}|{subject}".encode()).hexdigest()[:24]
        + f"@{domain}>"
    )
    return {"id": msg_id, "message": "Queued. Thank you. (dry-run)"}
